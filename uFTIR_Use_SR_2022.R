...
# I.  Installation (only for the 1st time) ####
  ...
  # * Install "uFTIR"
    remotes::install_github("SRebisz/uFTIR") #, ref=dev ref=mod_plot
...
# II. uFTIR Mosaic processing ####
  # *	Load packages 
    # devtools::load_all() <- use to update rewritten script 
    library(parallel)
    library(uFTIR)
  ...
  # * Set a sample name 
    File="pe"
    src <- File
    list.files(src)[1] #check the folder
  
  # * Load files .dmd   
    #Files.dmd=list.files(src)
    #Files.dmd=Files.dmd[grep(".dmd",Files.dmd)]
    Files.dmd=list.files(src, pattern=glob2rx("*.dmd"))
  
  # * mosaic_info .dmt
  x.mosa <- mosaic_info(dmtfile = file.path(File, paste(File,".dmt", sep = ""))) #paste("2_", File,"_pm.dmt", sep = "") basic information about the image, does not load the measured spectra
  
  # *  spectral angle mapper algorithm SAM ####
  #Spectral angle mapper algorithm chunk by chunk for the mosaics files
  system.time(outdir <- mosaic_sam(x.mosa, primpke, derivative = NULL, base_corr = FALSE, n_cores = NULL) ) #  FUN = function(x){prospectr::savitzkyGolay(x, 1, 3, 7)}) )
    
  # * Compose ####
   system.time(x.sam <-mosaic_compose(x.mosa@path, clusterlist = primpke@clusterlist, drop_raw=TRUE, nslices =3 )) # Load the SAM results in R
  
  par(mar=c(2,2,1,0))
  
  plot(x.sam, legend = FALSE)#,xlim=c(0,700), ylim=c(0,700))
  #dev.off()
  # * Smooth ####
  ...
  # * Export summary ####
    #original line x.sum_clip <- summary_sam(x.smooth, mask = clip_mask , clusternames = primpke@clusternames, slice=1, smooth = FALSE, temporal = TRUE)
    #adapted line x.sum_clip <- summary_sam(x.sam, mask = NULL , clusternames = primpke@clusternames, slice=1, smooth = FALSE, temporal = TRUE)
  ...
  # load the shape file in R
    ...
    x.shape <- sf::read_sf(paste(outdir, "/shape_out", sep = ""), "clusters" , verbose = F)
    
    #then you can do whatever you want with them. Here we subset and plot.
